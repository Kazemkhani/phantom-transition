"""CLI: run N seeded runs of one configuration against the installed the OpenAI Agents SDK.

    openai_agents/.venv/bin/python experiments/frameworks/openai_agents/run.py \
        --config sync-tool --n 50 --seed 20260902 --out results.jsonl

`--optout` forces the graceful cancel mode (after_turn) for any configuration,
for side measurements of what the SDK's run-scoped opt-out changes.
"""
...
async def _main(args: argparse.Namespace) -> int:
    ...
    from experiment import SDK_VERSION, run_one
